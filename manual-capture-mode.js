(()=>{
  ...
  let active=null,loggedIn=false,extensionReady=false,renderTimer=null;
  ...
  function setCard(id,status,text,data){
    ...
    const btn=card.querySelector(`[data-session-source="${id}"]`);if(btn){btn.disabled=false;btn.textContent=status==='capturing'?'Capturando…':'Capturar tela atual';}
    card.classList.remove('pending','capturing','complete','partial','error','unavailable');card.classList.add(status||'pending');
    const state=card.querySelector('.dfso-state');if(state)state.textContent=status==='complete'?'✅ Salvo':status==='partial'?'🟡 Parcial':status==='capturing'?'🔄 Captando':status==='error'?'⚠️ Erro':'○ Pendente';
    const p=card.querySelector('p');if(p)p.textContent=text||'Abra a fonte no Escola Total e capture a tela atual.';
    const meta=card.querySelector('.dfso-meta');if(meta&&data){meta.innerHTML=`<span>${Number(data.rows||0).toLocaleString('pt-BR')} registros</span><span>${Number(data.datasets||0)} conjunto(s)</span>${data.capturedAt?`<span>${fmt(data.capturedAt)}</span>`:''}${data.school?`<span>${clean(data.school)}</span>`:''}`;}
  }
  function apply(){
    document.getElementById('df-bootstrap-box')?.remove();
    const host=document.getElementById('df-session-center');if(!host)return;
    host.querySelectorAll('.dfso-controls label,[data-session-action="new"],[data-session-action="all"]').forEach(el=>el.style.display='none');
    const connect=host.querySelector('[data-session-action="connect"]');if(connect){connect.style.display='inline-flex';connect.textContent=loggedIn?'Escola Total conectado':'Conectar Escola Total';}
    ...
    info.innerHTML='<strong>Modo manual de captura</strong><br>1. Clique em Conectar. 2. No Escola Total, escolha manualmente a escola e abra a fonte desejada. 3. Volte aqui e clique em <b>Capturar tela atual</b>. O Diagnóstico não muda escola, fonte ou página sozinho.';
    for(const [id,def] of Object.entries(defs)){
      const card=sourceCard(id);if(!card)continue;
      const title=card.querySelector('.dfso-title b');if(title)title.textContent=def.label;
      const btn=card.querySelector(`[data-session-source="${id}"]`);if(btn){btn.disabled=false;btn.textContent=active?.sourceId===id?'Capturando…':'Capturar tela atual';}
  ...
  function schedule(){clearTimeout(renderTimer);renderTimer=setTimeout(apply,60)}

  function startCapture(sourceId){
    const def=defs[sourceId];if(!def)return;
    if(active)return;
  ...
  function finalize(sourceId,data,coverage='complete'){
    ...
    // Feed the existing intelligence view once, only after a valid capture is complete.
  ...
  window.addEventListener('message',e=>{
    ...
    if(d.type==='DF_READY'){extensionReady=true;schedule();return;}
    if(d.type==='DF_SESSION'){loggedIn=!!d.session?.loggedIn;schedule();return;}
    if(!active||d.requestId!==active.requestId)return;
    if(d.type==='DF_SOURCE_SESSION_PROGRESS'){active.text=clean(d.progress?.text||d.stage||'Captando a tela atual…');setCard(active.sourceId,'capturing',active.text);return;}
